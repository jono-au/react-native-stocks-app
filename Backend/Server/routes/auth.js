...
router.post('/register', validate, async (req, res) => {
    ...
    try {
        ...
        res.send({success: true,
            data: {id:savedUser._id, fullName: savedUser.fullName,email: savedUser.email,}, 
         token
        });
    }
    catch (error) {
...
router.post('/login', loginValidation, async (req, res) => {
    ...
    const token = generateToken(user);
    res.header("auth-token", token).send({success: true, data: {id:user._id, fullName: user.fullName, email: user.email,}, message: 'Logged in successfully', token})
});
